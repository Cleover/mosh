...
func TestLibraryAllURLRequestsReleaseType(t *testing.T) {
	...
	if !strings.Contains(url, "includeFields=thumbBlurHash,artBlurHash,titleSort,parentTitleSort,grandparentTitleSort,releasetype") {
		t.Fatalf("libraryAllURL() = %q; expected releasetype in includeFields", url)
	}
}
